...
- Each service automatically gets an `Error` class inheriting from `StandardError`
- `ExceptionWrapper` wraps service-originated errors with the service's `Error`
- Do not manually define service `Error` classes or rescue/reraise errors with them
- See: [lib/baseline/services/base_service.rb](lib/baseline/services/base_service.rb)
...
Avo, EmailValidator, FriendlyId, MJML, Money, Namespaces, Octokit, PaperTrail, PgHero, Postmark, Sentry
...
**Content:** MJML, Markdown parsers, FriendlyId
